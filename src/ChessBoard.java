...
public class ChessBoard extends JPanel implements ActionListener {
    ...
    public void forceSync()
    {
        ...
        if (LANChess.opponent == null) return;
        {
            for (int x = 0; x < 8; x++)
            {
                for (int y = 0; y < 8; y++)
                {
                    ...
                    try
                    {
                        url = new URL( "http://" + LANChess.opponent + "/set?position=" + position + "&value=" + square[y][x] + "&unmoved=" + unmoved[y][x]);
